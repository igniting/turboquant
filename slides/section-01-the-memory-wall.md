# Section 1: The Memory Wall — Why You Should Care

**Duration:** 5 minutes  
**Goal:** Make every engineer in the room feel the KV cache pain in dollar terms and hardware terms before you introduce any solution.

---

## Opening Hook

Start with a question to the audience:

> "How many of you have hit an out-of-memory error while running an LLM — either locally or in production?"

Most hands will go up. That shared experience is your entry point.

> "Today I'm going to explain exactly *where* that memory goes, and how a paper from Google Research makes most of it disappear — without losing a single correct answer."
...
## What Is the KV Cache? (One Sentence Version)
...
> **Key Insight:** At 32K context, the KV cache is as large as the model itself. At 128K context, it's 4× larger. The cache, not the model, becomes the bottleneck.
...
## The Bandwidth Problem (Bonus Pain)
...
What if you could compress every vector in the KV cache from 16 bits per coordinate down to 3-4 bits — a 4-5× reduction — with **zero loss in model quality**?
...
- **Open energy:** Start strong. The memory wall is something every engineer who's deployed an LLM has felt. Make them nod before you teach.
- **Use the tables:** Don't just say "the KV cache is big." Show the numbers. Engineers respond to concrete math.
- **The 512-user calculation** is your shock moment. Let it land. Pause after "8 TB."
- **Don't explain KV cache mechanics here** — just make them feel the pain. The next two sections will build the understanding.
- **End on curiosity:** "Three operations, backed by a proof" should make them lean forward.
- **Possible audience question:** "Can't we just use FP8 or INT8?" — Yes, but that's only 2× compression. TurboQuant gives you 4-5×. And FP8 still leaves substantial memory pressure at long contexts.
